...
class BertModel(BertPreTrainedModel):
  ...
  def __init__(self, config):
    ...
    num_dep_labels = 50
    self.dep_embedding = nn.Embedding(num_dep_labels, config.hidden_size)
  ...
  def embed(self, input_ids, POS_tag_ids = None, dep_tag_ids = None):
    ...
    # Dependency parse tree tag id
    if dep_tag_ids != None:
      dep_embeds = self.dep_embedding(dep_tag_ids) # (batch_size, seq_len, hidden_size)
      embeds += dep_embeds
  ...
  def forward(self, input_ids, attention_mask, POS_tag_ids = None, dep_tag_ids = None):
    """
    input_ids: [batch_size, seq_len], seq_len is the max length of the batch
    attention_mask: same size as input_ids, 1 represents non-padding tokens, 0 represents padding tokens
    """
    # get the embedding for each input token
    embedding_output = self.embed(input_ids=input_ids, POS_tag_ids = POS_tag_ids, dep_tag_ids = dep_tag_ids)
